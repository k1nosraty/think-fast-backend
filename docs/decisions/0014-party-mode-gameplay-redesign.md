...
2. **Server-Authoritative Evaluation and Zero-Leak Privacy**:
   ...
   - When a player solves, the server emits a lightweight `participant.solved` notification containing only the solver's identity, attempt count and solve duration. Rank and placement stay private until `round.finished` publishes the ranked summary.
